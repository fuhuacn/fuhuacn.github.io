...
![arp](/images/posts/knowledge/computerNetwork/arp.png)

## 网际控制报文协议 ICMP

ICMP 是为了更有效地转发 IP 数据报和提高交付成功的机会。它封装在 IP 数据报中，但是不属于高层协议。

![ICMP 1](/images/posts/knowledge/computerNetwork/icmp1.jpeg)
...
![NAT](/images/posts/knowledge/computerNetwork/nat.png)
...
![分组转发](/images/posts/knowledge/computerNetwork/分组转发.jpeg)
...
![四次挥手](/images/posts/knowledge/computerNetwork/四次挥手.jpeg)
...
只有在接收窗口向前滑动时（与此同时也发送了确认），发送窗口才有可能向前滑动。通过动态的滑动窗口大小，控制 TCP 的流量。
...
![开始](/images/posts/knowledge/computerNetwork/开始.jpeg)
...
![拥塞控制](/images/posts/knowledge/computerNetwork/拥塞控制.jpeg)

TCP 主要通过四个算法来进行拥塞控制：慢开始、拥塞避免、快重传、快恢复。

发送方需要维护一个叫做拥塞窗口（cwnd）的状态变量，**注意拥塞窗口与发送方窗口的区别：拥塞窗口只是一个状态变量，实际决定发送方能发送多少数据的是发送方窗口。**

每个发送方位维护两个窗口：一个是接收方准许的窗口，第二个是拥塞窗口。
...
1. 发送方原因：TCP 默认会使用 Nagle 算法。而 Nagle 算法主要做两件事：1）只有上一个分组得到确认，才会发送下一个分组；2）收集多个小分组，在一个确认到来时一起发送。所以，正是 Nagle 算法造成了发送方有可能造成粘包现象。
   
   Nagle 算法的规则（可参考tcp_output.c文件里tcp_nagle_check函数注释）：
...
1. 如果发送方发送的多个分组本来就是同一个数据的不同部分，比如一个很大的文件被分成多个分组发送，这时，当然**不需要处理**粘包的现象；
2. 如果多个分组本毫不相干，甚至是并列的关系，我们就一定要处理粘包问题了。比如，我当时要接收的每个分组都是一个有固定格式的商品信息，如果不处理粘包问题，每个读进来的分组我只会处理最前边的那个商品，后边的就会被丢弃。这显然不是我要的结果。

### 处理粘包现象方法

1. 发送方：对于发送方造成的粘包现象，我们可以通过**关闭** Nagle 算法来解决，使用 TCP_NODELAY 选项来关闭 Nagle 算法。
...
1. DHCP 配置 IP 和 DNS 服务器。
2. 拿网址找 DNS 服务器请求，返回的是 CNAME 再去找就近 CDN。
3. 如果是 HTTP 可能转发到 HTTPS，三次握手建立 TCP 连接，跟 HTTPS 做 SSL 认证、传 secret key
4. 发送 request 请求
5. controller 层解析请求，生成页面
6. 返回 response
7. 四次挥手断开连接
8. 浏览器解析返回的页面显示
