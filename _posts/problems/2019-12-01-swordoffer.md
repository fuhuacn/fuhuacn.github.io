...
+ 解法：
    ...
    关键就在如何推出递推公式。从 0-n 中删掉的第 m 个元素后，可以想像 m 元素后的那个值为新的 0，就变成了 0-n-1 删掉第 m 个元素。此时新的 0 对应的是老的 m（m-1 是删掉的，因为从 0 开始）。新的 n-1（也就是新的最后一个位置）位置对应的是 m-2（此时的 n 已经成 n-1 了）。所以可以看出递推公式是 ((n-1,m)+m)%n。
